...
class TestIsolate:
    ...
    def test_isolate_1p(self, empty_cobj):
        empty_cobj.data.points = cnn.Points([[1], [2], [3], [4]])
        empty_cobj.data.points.edges = [4]
        empty_cobj.labels = cnn.Labels([1, 1, 2, 2])
        empty_cobj.isolate()
        ...
        np.testing.assert_array_equal(
            empty_cobj.children[1]._refindex, [0, 1]
            )
        np.testing.assert_array_equal(
            empty_cobj.children[2]._refindex, [2, 3]
            )

    def test_isolate_2p(self, empty_cobj):
        empty_cobj.data.points = cnn.Points.from_parts([[[1], [2]], [[3], [4]]])
        np.testing.assert_array_equal(
            empty_cobj.data.points.shape,
            [2, 2]
            )
