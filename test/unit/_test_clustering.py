...
class TestCython:
    ...
    def test_fit_from_PointsArray_base_points(
        self, radius_cutoff, cnn_cutoff, base_data
    ):
        ...
        cfits.fit_from_PointsArray(points, labels, consider, radius_cutoff, cnn_cutoff)

        np.testing.assert_array_equal(np.array(base_data["labels"]), labels)
    ...
    def test_fit_from_NeighbourhoodsArray_base_neighbourhoods_e15(
        ...
        neighbourhoods = np.array(
            [np.asarray(x, dtype=np.int_) for x in base_data["neighbourhoods"]],
            dtype=object,
        )
    ...
    def test_fit_from_SparsegraphArray_base_densitygraphs_e15(
        ...
        indices = np.asarray(base_data["densitygraph"][1], dtype=cfits.ARRAYINDEX_DTYPE)
        labels = np.zeros(indices.shape[0] - 1, dtype=np.int_)
        consider = np.ones_like(labels, dtype=np.uint8)
        cfits.fit_from_SparsegraphArray(vertices, indices, labels, consider)
...
class TestCythonRegression:
    ...
    def test_fit_from_Points(
        self, radius_cutoff, cnn_cutoff, num_regression, fixed_circles_cobj
    ):
